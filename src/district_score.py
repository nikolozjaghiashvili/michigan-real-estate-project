...
def get_district_score(atrisk, enrollment, assessment, csv_path = 'resources/data/df_district_score.csv', csv_save = False):
    ...
    totalassessmentdf1["amalgam_education_score"] = ((1/totalassessmentdf1["CollegeNotReady"]) * (1/totalassessmentdf1["Total % Enrolled in an IHE within 0-6 months"]) * (1 / totalassessmentdf1["PercentMet_sci"]) * (1 / totalassessmentdf1["PercentMet_soc"]))
    totalassessmentdf1['amalgam_education_score'] = totalassessmentdf1['amalgam_education_score'].fillna((1/totalassessmentdf1["CollegeNotReady"]) * (1/totalassessmentdf1["Total % Enrolled in an IHE within 0-6 months"]) * (1 / totalassessmentdf1["PercentMet_sci"].mean()) * (1 / totalassessmentdf1["PercentMet_soc"].mean()))
    ...
    totalassessmentdf1 = totalassessmentdf1.rename(columns={'PercentMet_soc':'percent_met_soc_dist','Total % Enrolled in an IHE within 0-6 months':'total_enrolled_dist','DistrictCode':'district_code','PercentMet_sci':'percent_met_sci_dist', 'CollegeNotReady':'college_not_ready_dist', 'amalgam_education_score':'amalgam_education_score_dist','log_correct_education_score':'log_correct_education_score_dist'})
    totalassessmentdf1 = totalassessmentdf1.dropna(subset=['amalgam_education_score']).reset_index(drop = True)

    if csv_save:
        totalassessmentdf1.to_pickle(csv_path)
